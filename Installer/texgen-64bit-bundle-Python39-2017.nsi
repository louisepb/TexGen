...
Name "TexGen-64bit-Python3 ${VERSION}"
...
Section "TexGen (required)" ;No components page, name is not important
 ...
 ; File msvcr71.dll
  ...
  ClearErrors
  ReadRegStr $0 HKLM "SOFTWARE\Python\PythonCore\3.9\PythonPath" ""
  ${If} ${Errors}
    WriteRegStr HKLM "SOFTWARE\Python\PythonCore\3.9\PythonPath" "" "$INSTDIR\Python\libstd"
  ${Else}
    ${IF} $0 == ""
             WriteRegStr HKLM "SOFTWARE\Python\PythonCore\3.9\PythonPath" "" "$INSTDIR\Python\libstd"
        ${ELSE}
               DetailPrint "Python 39 path already exists"
        ${ENDIF}
  ${EndIf}
