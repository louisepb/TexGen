...
void CPeriodicBoundaries::SetupMaterials( CTextile &Textile )
{
	...
	m_Materials.clear();
	/*pair< CObjectContainer<CMaterial>, CObjectContainer<CMaterial> > DefaultMat;
	GetDefaultMaterial( DefaultMat );
	m_Materials["Default"] = DefaultMat;*/  // Need to sort out so common functions are in a sensible place to be 
																		// accessible to all Abaqus output routines
	...
	for( i = -1; i < iNumYarns; ++i )
	{
		...
		for (itMaterial = m_Materials.begin(); itMaterial != m_Materials.end(); ++itMaterial)
		{
			...
			//pair< CObjectContainer<CMaterial>, CObjectContainer<CMaterial> > Material = (CObjectContainer<CMaterial>)(itMaterial->second);
...
void CPeriodicBoundaries::CreateMaterials(ostream &Output, int iNumYarns, bool bMatrixOnly )
{
	...
	Output << "*****************" << endl;
	map<string, pair<CObjectContainer<CMaterial>, CObjectContainer<CMaterial> > > Materials = m_Materials;
	/*if (Materials.empty())
	{
		pair<CObjectContainer<CMaterial>, CObjectContainer<CMaterial> > DefaultMat;
		GetDefaultMaterial( DefaultMat );
...
void CPeriodicBoundaries::AssignMaterial(string Material, const vector<int> &Yarns)
{
...
void CPeriodicBoundaries::AddMaterial(string Name, const pair< vector<double>, vector<double> > &Constants)
{
...
//CMaterial &CPeriodicBoundaries::GetDefaultMaterial()
void CPeriodicBoundaries::GetDefaultMaterial( pair< CObjectContainer<CMaterial>, CObjectContainer<CMaterial> > &Material)
{
	pair< CKeywordMaterial, CKeywordMaterial > DefaultMat;
	//static CKeywordMaterial DefaultMat;
	ostringstream Output;
	Output << "*Elastic" << endl;
	Output << "1.0, 0.1" << endl;
	DefaultMat.first = CKeywordMaterial(Output.str());
	Output.clear();
	Output << "*Expansion" << endl;
	Output << "3e-06" << endl;
	DefaultMat.second = CKeywordMaterial(Output.str());
	Material = DefaultMat;
}

void CPeriodicBoundaries::AssignMaterial(string Material, int iYarn)
{
	m_MaterialAssignements[iYarn] = Material;
}

bool CPeriodicBoundaries::CheckYarnConstants( pair< vector<double>, vector<double> > &Constants )
{
...
bool CPeriodicBoundaries::CompareMaterialConstants( const vector<double> &MatConstants, const vector<double> &ThermConstants, const pair< vector<double>, vector<double> > &Constants )
{
